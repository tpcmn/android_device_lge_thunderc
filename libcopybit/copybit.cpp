...
/** setup mdp request */
static void set_infos(struct copybit_context_t *dev, struct mdp_blit_req *req, int mFlags) {
    req->alpha = dev->mAlpha;
    req->transp_mask = MDP_TRANSP_NOP;
    req->flags = dev->mFlags | mFlags;
}
        ...
        struct copybit_region_t const *region) 
{
    ...
    if (ctx) {
        ...
        if ((long)src_rect->l < 0 || (long)src_rect->r > (long)src->w ||
            (long)src_rect->t < 0 || (long)src_rect->b > (long)src->h) {
        ...
        while ((status == 0) && region->next(region, &clip)) {
            ...
            set_rects(ctx, req, dst_rect, src_rect, &clip, 0);
